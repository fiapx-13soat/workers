...
public class S3VideoStorage implements VideoStorage {
    ...
    public S3VideoStorage(S3Client s3, @Value("${workers.aws.s3.bucket-videos}") String bucket) {
        this.s3 = s3;
        this.bucket = bucket;
    }
    ...
    public Path download(String storageKey, Path targetDirectory) {
        ...
        try {
            s3.getObject(GetObjectRequest.builder().bucket(bucket).key(storageKey).build(), target);
            return target;
        } catch (NoSuchKeyException e) {
            // vídeo não existe → não adianta retry
            throw ProcessingException.deterministicFailure(
                    "VIDEO_NOT_FOUND", "O vídeo enviado não foi encontrado.", e);
        } catch (S3Exception | SdkClientException e) {
            // rede/servidor → transitório
            throw ProcessingException.transientFailure(
                    "S3_DOWNLOAD", "Falha ao obter o vídeo para processamento.", e);
        }
    }
